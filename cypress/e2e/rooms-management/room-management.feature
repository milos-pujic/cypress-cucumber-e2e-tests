Feature: Rooms Management
  ...
  Scenario Outline: User must be able to create new room by fillin up all mandatory fields
    When User creates new '<room_type>' type '<accesibility>' room '<room_name>' priced at '<room_price>' GBP with '<features>'
    Then New '<room_type>' type '<accesibility>' room '<room_name>' priced at '<room_price>' GBP with '<features>' will be created
    Examples:
      | room_name | room_type | room_price | accesibility   | features                                      |
